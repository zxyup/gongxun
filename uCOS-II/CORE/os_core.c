    ...
    if (OSRunning == OS_TRUE) {
        ...
        if (OSIntNesting == 0u) {                          /* Reschedule only if all ISRs complete ... */
            if (OSLockNesting == 0u) {                     /* ... and not locked.                      */
                OS_SchedNew();
                OSTCBHighRdy = OSTCBPrioTbl[OSPrioHighRdy];/* OSPrioHighRdy???OS_SchedNew(),??????????????OSTCBHighRdy*/
...
void  OSTimeTick (void)
{
    ...
    OSTimeTickHook();                                      /* Call user definable hook ????:??tick????????                    */
#endif
#if OS_TIME_GET_SET_EN > 0u
    OS_ENTER_CRITICAL();                                   /* Update the 32-bit tick counter               */
    OSTime++;
    OS_EXIT_CRITICAL();                                    /*??OS_ENTER_CRITICAL?OS_EXIT_CRITICAL???????*/
#endif
    if (OSRunning == OS_TRUE) {
#if OS_TICK_STEP_EN > 0u
        switch (OSTickStepState) {                         /* Determine whether we need to process a tick  ??OSTickStepState????step??*/
        ...
        if (step == OS_FALSE) {                            /* Return if waiting for step command ??step????          */
            return;
        }
#endif
